...
import { useAdministrativeDirectionForm } from "../hooks/use-administrative-direction-form";
...
export function AdministrativeDirectionForm({ defaultValues: item, onSuccess }) {
  const { form, formConfig, isPending, onSubmit } = useAdministrativeDirectionForm({ defaultValues: item, onSuccess });
            ...
            <div key={rowIndex} className="grid gap-4 md:grid-cols-2">
              {row.map((field) => {
                ...
                if (field.component === "switch") {
                  return (
                    <CustomFormSwitch key={field.name} control={form.control} {...field} />
                  );
                }
                return null;
              })}
